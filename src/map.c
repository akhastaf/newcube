...
void    render_wall()
{
    ...
    while (i < g_game.win_w)
    {
        ...
        wall_strip_h = (TILE_SIZE / correct_distance) * distance;
        top = (g_game.win_h / 2) - (wall_strip_h / 2);
        ...
        while (j < g_game.win_h)
        {
            ...
            if (j < top)
                color = g_game.ceil;
            else if (j < top + wall_strip_h)
            {
                g_game.tex[t].offset_y =  (j + ((wall_strip_h / 2) - (g_game.win_h / 2))) * ((float)(g_game.tex[t].h) / (float)wall_strip_h);
...
int     hit_wall(t_pos pos)
{
    ...
    if (g_player.walk_direction == 1 && g_player.rotation_angle >= M_PI && g_player.rotation_angle <= 2 * M_PI)
       index_y = (floor((pos.y - 1)) / TILE_SIZE);
    if (g_player.walk_direction == 1 && g_player.rotation_angle >= M_PI / 2 && g_player.rotation_angle <= 1.5 * M_PI)
        index_x = (floor((pos.x - 1)) / TILE_SIZE);
    if (g_player.walk_direction == 1 && g_player.rotation_angle >= 0 && g_player.rotation_angle <= M_PI)
       index_y = (floor((pos.y + 1)) / TILE_SIZE);
    if (g_player.walk_direction == 1 && g_player.rotation_angle >= 0 / 2 && g_player.rotation_angle <= 0.5 * M_PI)
        index_x = (floor((pos.x + 1)) / TILE_SIZE);
...
int     is_wall(float x, float y)
{
    ...
    if (g_game.map.map[(int)index_y][(int)index_x] == '1' || g_game.map.map[(int)index_y][(int)index_x] == ' ')
        return 1;
    return 0;
}